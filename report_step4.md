...
### Caching
- all responses from the destination server must be cached in a local directory. The html files name are generated by hashing the request URI. The proxy must check the cache before forwarding the request to the destination server. If a cached response is found, it should first check if it is still valid. If it is valid, the cached response should be served to the client instead of forwarding the request. If a cached response is not found, the proxy should forward the request to the destination server and cache the response for future requests.
